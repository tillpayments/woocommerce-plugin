    ...
    var injectGooglePay = function () {
        if (!injected) {
            ...
            $.getScript('https://pay.google.com/gp/p/js/pay.js')
                .done(function() {
                    injected = true;
                    toggleGooglePayButton($('input[type=radio][name=payment_method]:checked').val());
                })
                .fail(function() {
    ...
    var initGooglePay = function () {
        ...
        window.tillGooglePayLoader.setConfiguration({
            ...
            tokenizationSpecification: {
                ...
                parameters: {
                    'gateway': window.googlePay.gateway_name,
            ...
            errorCallback: function (err) {
                if (err.hasOwnProperty('statusCode') && err.statusCode === 'CANCELED') {
                    ...
                    $tillPaymentsErrors.html('An error occurred while processing the payment. Please check whether your entered data is valid.');
                    console.error('An error occurred while processing the payment. Please check whether your entered data is valid.');
                }
            }
        });
        window.tillGooglePayLoader.onGooglePayLoaded()
            .then(function() {
